	...
	public function postLogin(Request $request){
 		...
 		if (Auth::attempt(['email' => $email, 'password' => $password]))
        {
        	...
        	if(Auth::user()->status == 0) // status 0 is for disabled users
            {
            	...
            	return redirect()->back()->with('message','Your account has been blocked.Please contact site administrator')
                	->with('alert-class', 'alert-danger')
                	->withInput();
            }
            // is_deleted 'Y' is for deleted users
            if(Auth::user()->is_deleted == 'Y') 
            {
            	
            	auth()->logout(); // log user out
            	return redirect()->back()->with('message','Your account has been deleted.Please contact site administrator')
                	->with('alert-class', 'alert-danger')
                	->withInput();
            }
